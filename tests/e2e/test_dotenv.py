"""Agentic Graph Composer loads a `.env` file from cwd on every command invocation
...
def test_dotenv_is_loaded_from_cwd(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("AGC_TEST_VAR", raising=False)
    (tmp_path / ".env").write_text("AGC_TEST_VAR=from-dotenv\n")
    monkeypatch.chdir(tmp_path)

    result = CliRunner().invoke(main, ["validate", str(FIXTURE)])

    assert result.exit_code == 0
    assert os.environ["AGC_TEST_VAR"] == "from-dotenv"


def test_dotenv_does_not_override_a_real_env_var(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setenv("AGC_TEST_VAR", "from-shell")
    (tmp_path / ".env").write_text("AGC_TEST_VAR=from-dotenv\n")
    monkeypatch.chdir(tmp_path)
    ...
    assert result.exit_code == 0
    assert os.environ["AGC_TEST_VAR"] == "from-shell"
