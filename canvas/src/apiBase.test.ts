...
describe("resolveApiBase", () => {
  ...
  it("falls back to window.__AGENTDRAFT_API_BASE__ (bundled server, ADR-015) when VITE_API_BASE is unset", () => {
    window.__AGENTDRAFT_API_BASE__ = "http://127.0.0.1:54321";

    expect(resolveApiBase()).toBe("http://127.0.0.1:54321");
  });

  it("treats an empty window.__AGENTDRAFT_API_BASE__ as configured (same origin), not unset", () => {
    window.__AGENTDRAFT_API_BASE__ = "";
