"""Local project state store (ADR-010): one shared SQLite file under `.agc/`,
...
LOCAL_STORE_DIR = Path(".agc")
STATE_DB_PATH = LOCAL_STORE_DIR / "state.db"


def ensure_local_store_dir() -> Path:
    """Create `.agc/` (relative to cwd) if missing, and return `STATE_DB_PATH`."""
