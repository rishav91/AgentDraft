...
`examples/tools.py`/`examples/handlers.py` and `agentdraft init`'s generated templates both follow
this pattern - it's the standard, IDE-discoverable way LangChain itself expects tools/messages to
be built, not an AgentDraft-specific convention.

Since this means your own project imports `langchain_core` directly, pin it to match what your
installed `agent-draft` version actually requires - check `pip show langchain`'s `Requires-Dist`
line for the `langchain-core` range - rather than leaving it to resolve implicitly, so a future
`agent-draft` upgrade can't silently shift the `langchain-core` version your imports depend on out
from under you. `agentdraft init`'s generated `requirements.txt` already does this for you (§5).
...
`agentdraft init` also generates a `requirements.txt`, pinned to the exact `agent-draft` and
`langchain-core` versions installed when you scaffolded - the reproducibility half of §3's pinning
advice, done for you. If `agent-draft` isn't on your package index yet (e.g. you installed it from
