# Architecture - Agentic Graph Composer

See [README](README.md) for the doc map. See [PRD](PRD.md) for why this exists.

## 1. Design tenets

1. **No abstraction before a second real consumer.** Agentic Graph Composer compiles and runs directly against
   LangGraph. No execution-backend interface, no capability-negotiation layer, until AgentWeave
   exists and forces the seam (`ADR-003`). This extends to Agentic Graph Composer-owned local storage: no
...
4. **Compile to the real thing, not a re-implementation.** The compiler's output is a genuine
   LangGraph `StateGraph`. Agentic Graph Composer does not reimplement graph execution, tool calling, or LLM
...
Everything left of `LangGraph StateGraph` is Agentic Graph Composer. Everything right of it - graph execution,
...
| Observability instrumentation (Phase 3) | Emits OpenTelemetry spans by wrapping each compiled node's function at compile time (`ADR-011`): one root span per run, one child span per node (`FR-7.1`, `FR-7.2`) | `opentelemetry-sdk`, `opentelemetry-exporter-otlp-proto-http` | No inbound interface - the wrapper is applied in `compile_schema`, the same extension point used for visit-tracking (`FR-1.12`); export destination is entirely env-var driven (`FR-7.3`, [OBSERVABILITY.md](OBSERVABILITY.md)) |
| Eval runner (Phase 3) | Compiles a schema once, runs each case from an evals file, checks deterministic assertions against final state (`FR-8.1`-`FR-8.4`) | Python, reuses the compiler/loader - no new execution path | `agc eval <schema> <evals-file>`; exits `4` on assertion failure (`ADR-012`) |
...
### 4.5 `agc explain <schema> --format json` + canvas (Phase 2.1)
...
### 4.6 `agc canvas <schema>` + canvas editing (Phase 2.2)
...
### 4.7 `agc run <schema> --resume <thread_id>` (Phase 3.1)
...
2. Instead of invoking from `START`, the compiled graph is invoked against the given `thread_id`;
   ...
   thread (`ADR-009`) - Agentic Graph Composer does not implement resume logic itself.
...
### 4.8 `agc eval <schema> <evals-file>` (Phase 3.5)
...
Not applicable at this stage. Agentic Graph Composer runs locally, on demand, for one user, on agents small
...
| LangGraph itself raises during execution (e.g. a tool call fails, an LLM call errors) | Agentic Graph Composer does not catch/reinterpret LangGraph's own runtime errors | LangGraph's native error surfaces as-is - Agentic Graph Composer is a thin compiler, not a runtime wrapper, per tenet 4 |
| LangGraph upstream breaking change | Compiler may fail against a newer LangGraph version | Agentic Graph Composer pins a tested LangGraph version per release (`ADR-003` consequence) rather than tracking latest |
| `agc run --resume` given an unknown/nonexistent `thread_id` (Phase 3) | Fails before any execution starts | A specific "no checkpoint found for thread_id" error, not a bare LangGraph traceback (`FR-5.3`) |
| `agc run --resume` given a schema that changed since that thread's last recorded run (Phase 3) | Fails before any execution starts, unless `--force` | A specific "schema has changed since thread_id's last run" error (`FR-5.6`, `ADR-014`); silently skipped (not enforced) if no prior run is recorded for that thread |
| Process killed mid-run with `checkpointer` configured (Phase 3) | The run ledger row is left at `status: running`; the checkpointer's own last-persisted checkpoint (if any) still exists | `agc runs list` reconciles a stale `running` row to `interrupted` at read time ([DATA-MODEL §4](DATA-MODEL.md#4-consistency--concurrency)); the user resumes with `--resume <thread_id>` if a checkpoint exists, or re-runs otherwise |
| `checkpointer.backend: postgres` configured but the database is unreachable (Phase 3) | Compilation succeeds; execution fails on first checkpoint write | LangGraph's own connection error surfaces, per tenet 4 - Agentic Graph Composer does not reinterpret it |
...
- **Security:** the custom-code escape hatch (`FR-1.6`) executes arbitrary local Python by design —
  no sandboxing in Phase 1 (deferred, `NFR-4.1`). Acceptable because Agentic Graph Composer is local-only,
...
- **Config/secrets:** LLM API keys and similar secrets are read from the environment (e.g.
  ...
  (`OTEL_EXPORTER_OTLP_ENDPOINT` etc., `FR-7.3`) - no Agentic Graph Composer-specific config file.
- **Idempotency/consistency (Phase 1-2):** not applicable - no persisted state to keep consistent.
  **Phase 3:** checkpoint resume covers graph *state* only, not the real-world side effects of
  already-executed custom-code nodes; Agentic Graph Composer does not attempt at-least-once/exactly-once
...
| Crash/resume | `agc run --resume` (Phase 3) | An e2e test kills the process mid-run and asserts `--resume` continues correctly (`NFR-7.1`) |
| Eval fixtures | `agc eval` (Phase 3) | Fixture evals files with known-passing and known-failing cases, asserting the correct exit code (`0` vs. `4`, `FR-8.4`) and correct per-case pass/fail reporting |
...
Vitest unit/component tests, and one Playwright e2e test that spawns the real `agc canvas`
server (not a mock) and drives an actual add-node/wire-edge/save round trip through a real
browser, asserting the schema file on disk changed and still passes `agc validate`. This is
